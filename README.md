...
- **통합 자산 관리 (Total Assets)**: 현재 **4개**의 핵심 엔터프라이즈 자산 실시간 추적 중.
  - **IT 인프라**: 서버 및 네트워크 장비 2개 (보안 감사 서버, 백본 스위치).
  - **OT/IoT 스마트 야드**: 생산 현장 장비 2개 (도크 온도 센서, 조립 로봇 컨트롤러).
- **실시간 장애 감지**: **Dock 1 온도 센서(IOT-DOCK1-TEMP-01)**의 이상 수치를 성공적으로 감지하여 경고(Warning) 상태로 분류.
- **AX 기반 의사결정 지원**: 
